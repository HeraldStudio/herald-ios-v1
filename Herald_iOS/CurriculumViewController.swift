...
class CurriculumViewController : BaseViewController {
    ...
    func readLocal () {
        ...
        do {
            ...
            var sidebarLecturer : [String:String] = [:]
            var sidebarCredit : [String:String] = [:]
            
            // 将课程的授课教师和学分信息放入键值对
            let sidebarArray = JSON.parse(sidebar)
            for i in 0 ..< sidebarArray.count {
                let obj = sidebarArray[i]
                sidebarLecturer.updateValue(obj["lecturer"].stringValue, forKey: obj["course"].stringValue)
                sidebarCredit.updateValue(obj["lecturer"].stringValue, forKey: obj["credit"].stringValue)
            }
            
            // 读取开学日期
            let startMonth = content["startdate"]["month"].int
            let startDate = content["startdate"]["day"].int
            var cal = NSDate()
            var format = NSDateFormatter()
            format.dateFormat = "yyyy/M/d"
            format.dateFromString("\(startMonth)/\(startDate)")
        }
    }
}
/*
class CurriculumViewController: UIViewController, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad();
        self.title = "课表助手";
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(title: "刷新", style: UIBarButtonItemStyle.Plain, target: self, action: #selector(CurriculumViewController.loadData));
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "设置", style: UIBarButtonItemStyle.Plain, target: self, action: #selector(CurriculumViewController.settings));
